...
export function listWines(query) {
...
export function updateWine(id, input) {
...
export function consumeWine(id) {
    const wine = getWineById(id);
    if (wine.quantity <= 0)
        throw new HttpError(409, "No bottle left in stock");
...
function buildWhere(query) {
    ...
    if (query.search) {
        ...
        const searchClause = or(like(wines.estate, pattern), like(wines.cuvee, pattern), like(wines.appellation, pattern), like(wines.region, pattern), like(wines.country, pattern), like(wines.grapes, pattern), like(wines.comments, pattern));
        if (searchClause)
    ...
    for (const [key, value] of Object.entries(query.filters)) {
        if (value === "" || value === null || value === undefined)
            continue;
        if (!(key in filterColumns))
            continue;
        const column = filterColumns[key];
        clauses.push(eq(column, (key === "vintage" ? Number(value) : String(value))));
    }
    return clauses.length ? and(...clauses) : undefined;
}
function normalizeWineInput(input, createdAt, updatedAt) {
