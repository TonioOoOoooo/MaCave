import type { NewWine } from "./schema.js";
...
export function parseInteger(value: unknown, zeroIsNull = false): number | null {
...
export function parseFrenchDate(value: unknown): string | null {
  const text = clean(value);
  if (!text) return null;
...
export function mapViniouRow(row: Record<string, string>): NewWine {
